# NVIDIA Omniverse on AWS — 디지털 트윈 구축 가이드 CDK 프로젝트
...
- 시사점: 온프렘은 보조(NIM)만 되고 본체(Kit)가 막힌 상태였음 → 본 구축은
...
- ⚠️ 실증 확인(2026-05-30): `docker compose --profile lite up --build`는 외부 일반
  ...
  - 즉 Blueprint 전체 스택은 "임의 호스트에서 compose up --build"로 안 됨.
    ...
    `deploy/aws-cdk`(prebuilt 이미지 pull 가정) 사용. → 본 구축 범위 밖, 별도 과제.
...
- 구축 전략: (1) lite로 Blueprint 구동·스트리밍 파이프라인 검증 → (2) 시각화 스택을
...
## 0-2. 최종 사양 (Omniverse NIM CAE 구축) — 교차 대조 결과
...
2. 인스턴스 1순위 재검토:
   ...
   - 따라서 g7e.12xlarge(96GB ×2)는 단일 GPU 구성엔 과사양. g7e.4xlarge(96GB ×1)로
...
5. 디스크: 사양서 500GB gp3 적정. (NIM 컨테이너 ~31GB + 샘플 USD + 캐시 여유 포함).
   루트/데이터 분리는 선택(섹션 4). 단일 볼륨 500GB도 무방.
...
> Phase 1은 학습 단계를 건너뛰고 pre-trained NIM(DoMINO-Automotive-Aero)을
...
|--------|-----|-----------|------|--------|------|
| `g7e.4xlarge` (기본) | RTX PRO 6000 ×1 | 96GB | 16 | 128 GB | 기본 구성. 단일 GPU로 NIM+Kit 풀스택, 비용 최적 |
...
본 구성은 단일 pre-trained NIM + Kit 구동 규모라, 루트 볼륨 하나(500GB gp3)로 통합한다.
(NIM 이미지 ~30GB + 모델 캐시 수십 GB + 샘플 USD + Kit 캐시 다 합쳐도 100~200GB 수준
→ 500GB로 충분한 여유). 별도 데이터 볼륨 분리는 불필요.
...
단일 GPU(g7e.4xlarge, 96GB)는 NIM(~40GB)+Kit(~15GB) 공존 OK라 기본 구성엔 충분.
...
본 구성은 단일 EC2에 컨테이너를 직접 실행(`docker run`)하는 방식으로, 검증·데모에
...
- 두 경로 모두 GPU 쿼터 사전 확인, NGC Key 등 시크릿의 안전 관리(K8s Secrets/IAM),
  ...
  프랙티스로 제시한다.
- 본 구성의 단일 EC2 방식은 이 둘로 가기 전 "NIM+Kit 파이프라인이 동작함"을 빠르게
...
- TLS 인증서 (8443 HTTPS):
  ...
  - ⚠️ ACM 인증서는 ALB/NLB/CloudFront에만 부착 가능(EC2 직접 불가). 본 구성은 고정 IP
...
본 구축은 "설치/구동 검증"이 목적이므로 별도 웹 IDE(code-server)는 넣지 않는다.
...
- code-server가 필요해지는 경우(현재 범위 밖): Kit 익스텐션 본격 개발(대량 Python
...
- DCV 자체에 GUI 파일 업/다운로드가 내장 → 일반 작업엔 충분.
...
- 용도: 성능 병목 파악, VRAM 부족(Kit+NIM 공존) 진단 → 인스턴스 상향 판단 근거.
...
- 고객 안내: 키는 본인 NGC 계정으로 직접 발급해야 하며 공유 불가.
  본 구성에서는 발급된 키를 Secrets Manager에 저장 → UserData가 조회해
...
구축 검증 완료(전 기능 실증). 아래는 운영/확장 단계에서 검토할 항목 — 지금은 불필요.
