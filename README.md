...
## Nucleus 협업 서버 (옵션 — 다중 사용자 동시 편집 시)

현재 구성은 로컬 USD 파일 사용(단일 사용자). 여러 사용자가 같은 씬을 실시간
동시 편집하려면 Nucleus Enterprise를 별도 인스턴스로 추가합니다.

```
[기존 g7e Kit+NIM+DCV] ──omniverse://nucleus-ip/scene.usd──→ [Nucleus EC2 (m5.xlarge)]
```

- Kit은 Nucleus 프로토콜 내장 → 코드 변경 없이 접속 URL만 변경
- 같은 VPC 내 프라이빗 통신, SG에 TCP 3009/3019/3020/3030 허용
- Nucleus EC2: GPU 불필요, m5.xlarge 정도 (~$0.23/hr)
- 라이선스: Nucleus Enterprise 자체 호스팅 가능 여부 사전 확인 필요
- 상세: `CLAUDE.md` 섹션 13 (향후 과제 — Nucleus 협업 서버)
