# Omniverse NIM CAE 구축 가이드 — 최종 사양서
...
- 본 구축 = "AI 유동해석 + Omniverse 3D 렌더링 파이프라인이 AWS에서 동작함"을 검증.
...
- 계층 번호(①②④⑤⑥)는 CLAUDE.md 섹션 5-0의 패키지 스택과 대응 (③ Kit 빌드 도구는
  kit-app-template 직접 빌드 시에만 — 본 구성은 Kit-CAE 직접 설치라 선택).
...
> 프로덕션 배포 참고: 본 구성은 단일 EC2에 NIM 컨테이너를 직접 실행하는 검증용 구성이다.
...
| 항목 | 비용 | 비고 |
|------|------|------|
| g7e.4xlarge (1순위, 서울) | $4.916/hr | 월 160h(8h×20일) ≈ $787 |
| g7e.12xlarge (2순위, 서울) | $10.187/hr | 월 160h ≈ $1,630 |
| g6e.12xlarge (3순위, 서울) | $12.900/hr | 월 160h ≈ $2,064 |
| EBS 500GB gp3 | ~$40/월 | 용량 기준 |
| S3 (샘플 데이터) | <$1/월 | |
| 미사용 시 EC2 중지 | $0 (EBS만 과금) | 수동 stop/start |

- 권장(g7e.4xlarge) 월 160시간(8h×20일) 사용 시 ≈ $827/월 (인스턴스 $787 + EBS $40).
...
다중 사용자가 같은 USD 씬을 실시간 동시 편집하려면 Nucleus Enterprise 추가.
...
| 항목 | 내용 |
|------|------|
| 인스턴스 | m5.xlarge (4vCPU/16GB, GPU 불필요) ~$0.23/hr |
| 통신 | 같은 VPC 프라이빗, SG TCP 3009/3019/3020/3030 허용 |
| Kit 변경 | 없음 (Nucleus 프로토콜 내장, URL만 변경) |
| 라이선스 | Nucleus Enterprise 자체 호스팅 가능 여부 확인 필요 |
| 실익 분기 | 2명 이상 동시 편집 시 가치. 1명이면 로컬 파일로 충분 |
