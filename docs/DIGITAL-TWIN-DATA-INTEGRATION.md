...
작성일: 2026-05-30
전제: 본 문서는 기존 구축 스택(NVIDIA Omniverse Kit/RTX + DCV/WebRTC, AWS CDK)을
"트윈 뷰·렌더·화면전송" 기반으로 재활용하고, 그 앞단에 OT 센서 데이터 연동
파이프라인을 신규로 붙이는 것을 전제로 한다.
대상 영역: 제조 플랜트 설비 디지털 트윈 모니터링.

> 위치: 현 구성(CAE 유동해석 시각화) 범위 밖의 확장 과제. 방향 확정 시
...
색 구분: 🟦 현장 OT(고객망) · 🟧 신규 구축 · 🟩 기존 스택 재활용 · ⬜ 옵션(대규모 시)
...
flowchart TB
  ...
  subgraph AWS["AWS — 기존 EC2 (g7e / g6e, 단일 인스턴스)"]
    direction TB
...
| 옵션 | 구성 | 장점 | 단점 |
|------|------|------|------|
| A. Omniverse 통합 | 3D 뷰 + 게이지/차트 UI를 Kit 앱 한 화면에 (Kit 확장 개발) | RTX 트윈과 데이터 완전 통합, WebRTC로 그대로 시연 | Kit 확장 개발 공수, 현재 범위 밖 |
...
- 본 방안은 현 구축의 확장 과제 → 방향 확정 시 CLAUDE.md 13으로 승격.
